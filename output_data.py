...
class TimetableExporter:
    def __init__(self):
        """Initialize the TimetableExporter"""
        self.output_dir = "timetable_exports"
    ...
    def get_timetable_data(self):
        """Get timetable data - prioritize passed data over file loading"""
        if self.timetable_data:
            return self.timetable_data
            
        # Fallback to loading from file
        try:
            with open('best_timetable.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
    ...
    def export_sst_timetables(self, timetable_data=None):
        """Export SST (engineering) timetables
        
        Args:
            timetable_data: Optional timetable data to export. If None, loads from file.
        """
    ...
    def export_tyd_timetables(self, timetable_data=None):
        """Export TYD (non-engineering) timetables
    ...
    def export_lecturer_timetables(self, timetable_data=None):
        """Export all lecturer timetables
        ...
        try:
                ...
                for time_slot_idx, row_data in enumerate(timetable_rows):
                    for day_idx in range(len(self.days)):
                        if day_idx + 1 < len(row_data):  # Skip time column
                            ...
                            if lecturer_info and lecturer_info['course_code']:
                                lecturer_name = lecturer_info.get('faculty', 'Unknown')
            ...
            # Create combined sheet for all lecturers
            self.create_combined_lecturer_sheet(wb, "Lecturer Timetables", lecturer_schedules)
